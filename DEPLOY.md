...
| GitHub 仓库 | 已推送：`github.com/gamesushi/blockswiki`（本仓库） |
...
git clone https://github.com/gamesushi/blockswiki.git /tmp/lw && \
...
2. 装 nginx + certbot；
3. `git clone` 仓库到 `/opt/blockswiki`；
4. 用 `openssl rand` 生成**随机密钥**写入 `/opt/blockswiki/.env`（gitignored，不在仓库里）；
...
验证：
```bash
cd /opt/blockswiki && sudo docker compose ps
...
sudo docker run --rm -v blockswiki_strapi-data:/data -v $PWD/api/.tmp:/src \
...
1. Vercel 控制台 **Add New > Project**，导入 `gamesushi/blockswiki`。
...
- 后端 `CORS_ORIGINS` = 前端域名（setup 脚本的 `--cors` 已设，如 `https://blockswiki.yourdomain.com`）。
- 改 CORS 后重启：`cd /opt/blockswiki && sudo docker compose restart strapi`。
...
| 变量 | 在哪设 | 作用 |
|---|---|---|
| `APP_KEYS` 等 6 个密钥 | VM 的 `/opt/blockswiki/.env` | Strapi 运行必需，脚本随机生成 |
...
```bash
# 后端（VM 上）
cd /opt/blockswiki && sudo git pull && sudo docker compose up -d --build
